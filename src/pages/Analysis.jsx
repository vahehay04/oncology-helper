...
{"items":[{"item":"название","status":"рекомендовано","comment":"краткий комментарий","source":"Минздрав","source_reference":"https://cr.minzdrav.gov.ru/..."}],"missing":[{"item":"название","recommendation":"кратко","source_reference":"url"}]}`;
    ...
    const STRICT_RULES = `
РЕЖИМ: СТРОГИЙ НОРМАТИВНЫЙ АНАЛИЗ КЛИНИЧЕСКОГО СЛУЧАЯ.

ОБЯЗАТЕЛЬНЫЕ ПРАВИЛА (нарушение недопустимо):
1. Используй ТОЛЬКО данные из официальных источников: rosoncoweb.ru, cr.minzdrav.gov.ru, nccn.org
2. ЗАПРЕЩЕНО использовать внутренние знания модели без подтверждения ссылкой на источник
3. ЗАПРЕЩЕНО использовать вероятностные формулировки ("возможно", "вероятно", "как правило")
4. ЗАПРЕЩЕНО генерировать альтернативные сценарии, не описанные в рекомендациях
5. Анализ должен быть ДЕТЕРМИНИРОВАННЫМ — для одних и тех же данных всегда один и тот же результат
6. Каждый вывод должен быть обоснован КОНКРЕТНЫМ пунктом из конкретного документа
7. Если в источнике нет прямого указания — пиши "Данный параметр не регламентирован в указанном источнике"
8. ЗАПРЕЩЕНЫ домены: Wikipedia, PubMed, UpToDate, Medscape, любые иные медицинские порталы
`;
