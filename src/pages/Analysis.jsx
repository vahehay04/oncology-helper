...
export default function Analysis() {
  ...
  const runAnalysis = async (caseData) => {
    ...
    const prompt = `Ты — AI-эксперт по онкологии. Проанализируй клинический случай пациента по актуальным клиническим рекомендациям.

ИСТОЧНИКИ для анализа:
1. Клинические рекомендации Минздрава РФ по меланоме кожи (ID 921, 2024) — https://cr.minzdrav.gov.ru/preview-cr/921_1
2. Рекомендации RUSSCO 2025 — https://rosoncoweb.ru/standarts/RUSSCO/
3. ESMO Clinical Practice Guideline for cutaneous melanoma (2025) — https://www.annalsofoncology.org/article/S0923-7534(24)04912-3/fulltext
...
ЗАДАНИЕ:
Проанализируй каждый пункт диагностики и лечения по клиническим рекомендациям и верни JSON.

Для КАЖДОГО пункта определи:
- "status": одно из "рекомендовано" (уровень доказательности A, B), "сомнительно" (уровень C), "не_рекомендовано" (уровень D, E или прямо написано "не рекомендуется")
- "evidence_level": уровень доказательности (A, B, C, D, E)
- "comment": подробный комментарий
- "source_reference": ссылка на конкретный раздел рекомендаций
- "source_text": цитата из клин. рекомендации

Также определи НЕДОСТАЮЩИЕ пункты - что не было указано врачом, но рекомендуется по клин. рекомендациям для данной стадии.
