...
1. cr.minzdrav.gov.ru
2. rosoncoweb.ru
3. nccn.org
4. rosoncoweb.ru/standarts/foreign/

Запрещены: annalsofoncology.org, esmo.org, pubmed.ncbi.nlm.nih.gov, uptodate.com, old.oncology-association.ru, 24radiology.ru, Wikipedia и любые другие.

Перед каждым пунктом анализа ты ОБЯЗАН:
1. Определить документ из разрешённого домена.
2. Убедиться, что домен точно совпадает с разрешённым.
3. Если не совпадает — написать "Источник не соответствует утверждённому перечню" в source_reference.
...
ЗАДАНИЕ:
Проанализируй каждый пункт диагностики и лечения и верни JSON.

Для КАЖДОГО пункта определи:
- "status": одно из "рекомендовано", "сомнительно", "не_рекомендовано"
- "evidence_level": уровень доказательности (A, B, C, D)
- "comment": подробный комментарий строго по тексту документа
- "source_domain": точный домен источника (например: cr.minzdrav.gov.ru)
- "source_reference": ПОЛНЫЙ URL страницы из разрешённого домена
- "source_document": официальное название документа и раздел/пункт
- "source_text": прямая цитата из рекомендации

Также определи НЕДОСТАЮЩИЕ пункты.
Оцени общее соответствие: "высокое", "среднее", "низкое".
В поле "summary" укажи: какие документы использованы (домен + URL + название).`;
