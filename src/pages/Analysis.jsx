...
`;
    ...
    const useInternet = !guidelineText;

    const buildDiagPrompt = (sourceName, sourceUrl, extraInstruction = "") => `${STRICT_RULES}
${guidelineContext}
ИСТОЧНИК ДЛЯ ЭТОГО АНАЛИЗА: ${sourceName}
ВХОДНЫЕ ДАННЫЕ ПАЦИЕНТА:
${caseContext}

ВЫПОЛНЕННАЯ ДИАГНОСТИКА — проверить КАЖДЫЙ пункт:
${diagnosticsList}
...
- comment: краткий вывод до 100 символов
- source: "${sourceName}"
- source_reference: "${sourceUrl}"

${jsonInstruction}`;

    const buildTreatPrompt = (sourceName, sourceUrl, extraInstruction = "") => `${STRICT_RULES}
${guidelineContext}
ИСТОЧНИК ДЛЯ ЭТОГО АНАЛИЗА: ${sourceName}
ВХОДНЫЕ ДАННЫЕ ПАЦИЕНТА:
${caseContext}
...
ПРОВЕДЁННОЕ ЛЕЧЕНИЕ — проверить КАЖДЫЙ пункт:
${treatmentsList}
...
${jsonInstruction}`;
    ...
    const russcoUrl = "https://rosoncoweb.ru/";
    const minzdravUrl = guidelineUrl || "https://cr.minzdrav.gov.ru/";
    const nccnUrl = "https://www.nccn.org/guidelines/";

    // Run all 6 analysis calls + summary in parallel
    const [
      russcodiagResult, russcoTreatResult,
      minzdravDiagResult, minzdravTreatResult,
      nccnDiagResult, nccnTreatResult,
      summaryResult
    ] = await Promise.all([
      // RUSSCO diagnostics
      diagnosticsList ? base44.integrations.Core.InvokeLLM({
        prompt: buildDiagPrompt("RUSSCO", russcoUrl, "Используй знания рекомендаций RUSSCO (rosoncoweb.ru) для данной нозологии."),
      ...
      treatmentsList ? base44.integrations.Core.InvokeLLM({
        prompt: buildTreatPrompt("RUSSCO", russcoUrl, "Используй знания рекомендаций RUSSCO (rosoncoweb.ru) для данной нозологии."),
        add_context_from_internet: true,
        response_json_schema: { type: "object", properties: { items: { type: "array", items: { type: "object" } }, missing: { type: "array", items: { type: "object" } } } }
      }) : Promise.resolve({ items: [], missing: [] }),

      // Minzdrav diagnostics
      diagnosticsList ? base44.integrations.Core.InvokeLLM({
        prompt: buildDiagPrompt("Минздрав", minzdravUrl, `Используй знания клинических рекомендаций Минздрава РФ (cr.minzdrav.gov.ru) для МКБ ${caseData.mkb_code || "данной нозологии"}.`),
        add_context_from_internet: true,
        response_json_schema: { type: "object", properties: { items: { type: "array", items: { type: "object" } }, missing: { type: "array", items: { type: "object" } } } }
      }) : Promise.resolve({ items: [], missing: [] }),
      ...
      // Minzdrav treatment
      treatmentsList ? base44.integrations.Core.InvokeLLM({
        prompt: buildTreatPrompt("Минздрав", minzdravUrl, `Используй знания клинических рекомендаций Минздрава РФ (cr.minzdrav.gov.ru) для МКБ ${caseData.mkb_code || "данной нозологии"}.`),
        add_context_from_internet: true,
        response_json_schema: { type: "object", properties: { items: { type: "array", items: { type: "object" } }, missing: { type: "array", items: { type: "object" } } } }
      }) : Promise.resolve({ items: [], missing: [] }),
      ...
      // NCCN diagnostics
      diagnosticsList ? base44.integrations.Core.InvokeLLM({
        prompt: buildDiagPrompt("NCCN", nccnUrl, "Используй знания рекомендаций NCCN (nccn.org) для данной нозологии."),
        add_context_from_internet: true,
        response_json_schema: { type: "object", properties: { items: { type: "array", items: { type: "object" } }, missing: { type: "array", items: { type: "object" } } } }
      }) : Promise.resolve({ items: [], missing: [] }),
      ...
      // NCCN treatment
      treatmentsList ? base44.integrations.Core.InvokeLLM({
        prompt: buildTreatPrompt("NCCN", nccnUrl, "Используй знания рекомендаций NCCN (nccn.org) для данной нозологии."),
        add_context_from_internet: true,
        response_json_schema: { type: "object", properties: { items: { type: "array", items: { type: "object" } }, missing: { type: "array", items: { type: "object" } } } }
      }) : Promise.resolve({ items: [], missing: [] }),
...
ЗАДАЧА: Нормативное заключение по RUSSCO, Минздраву РФ и NCCN.
- overall_compliance: "высокое" (>=80%), "среднее" (50-79%), "низкое" (<50%)
- summary: 3-4 предложения строго по данным
...
Верни JSON: {"overall_compliance":"высокое|среднее|низкое","summary":"..."}`,
    ...
    // Merge results, tag each item with its source
    const tagItems = (result, sourceName, analysisType) =>
      (result?.items || []).map(i => ({ ...i, source: sourceName, analysis_type: analysisType }));
    const tagMissing = (result, sourceName, analysisType) =>
      (result?.missing || []).map(i => ({ ...i, source: sourceName, analysis_type: analysisType }));

    const diagResult = {
      items: [
        ...tagItems(russcodiagResult, "RUSSCO", "диагностика"),
        ...tagItems(minzdravDiagResult, "Минздрав", "диагностика"),
        ...tagItems(nccnDiagResult, "NCCN", "диагностика"),
      ],
      missing: [
        ...tagMissing(russcodiagResult, "RUSSCO", "диагностика"),
        ...tagMissing(minzdravDiagResult, "Минздрав", "диагностика"),
        ...tagMissing(nccnDiagResult, "NCCN", "диагностика"),
      ]
    };
    const treatResult = {
      items: [
        ...tagItems(russcoTreatResult, "RUSSCO", "лечение"),
        ...tagItems(minzdravTreatResult, "Минздрав", "лечение"),
        ...tagItems(nccnTreatResult, "NCCN", "лечение"),
      ],
      missing: [
        ...tagMissing(russcoTreatResult, "RUSSCO", "лечение"),
        ...tagMissing(minzdravTreatResult, "Минздрав", "лечение"),
        ...tagMissing(nccnTreatResult, "NCCN", "лечение"),
      ]
    };
