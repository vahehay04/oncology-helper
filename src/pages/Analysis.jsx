...
{"items":[{"item":"название","status":"рекомендовано","comment":"краткий комментарий","source":"Минздрав","source_reference":"https://cr.minzdrav.gov.ru/..."}],"missing":[{"item":"название","recommendation":"кратко","source_reference":"url"}]}`;
    ...
    const [diagResult, treatResult, summaryResult] = await Promise.all([
      diagnosticsList ? base44.integrations.Core.InvokeLLM({
        prompt: `Ты — AI-эксперт по онкологии. Проверь КАЖДЫЙ пункт диагностики на соответствие клиническим рекомендациям RUSSCO (rosoncoweb.ru) — раздел "Диагностика".

${caseContext}

ВЫПОЛНЕННАЯ ДИАГНОСТИКА (проверь КАЖДЫЙ пункт, не пропускай ни один):
${diagnosticsList}

ВАЖНО: Биопсия, гистологическое исследование, ИГХ, молекулярно-генетические исследования — это обязательные пункты, их нельзя игнорировать. Анализируй каждый из них отдельно.

Для КАЖДОГО пункта диагностики укажи:
- item: точное название из списка
- status: "рекомендовано" / "сомнительно" / "не_рекомендовано"
- comment: 1-2 предложения — соответствует ли этот вид обследования разделу "Диагностика" рекомендаций RUSSCO для данного типа рака
- source: "RUSSCO"
- source_reference: URL вида https://rosoncoweb.ru/standarts/ (конкретный документ RUSSCO для данного МКБ-кода)

В missing — обследования из раздела "Диагностика" RUSSCO которые должны были быть выполнены, но отсутствуют в списке.
...
${jsonInstruction}`,
      ...
      treatmentsList ? base44.integrations.Core.InvokeLLM({
        prompt: `Ты — AI-эксперт по онкологии. Детально проверь КАЖДЫЙ пункт лечения на соответствие клиническим рекомендациям RUSSCO (rosoncoweb.ru), Минздрава РФ (cr.minzdrav.gov.ru) и NCCN (nccn.org).

${caseContext}

ПРОВЕДЁННОЕ ЛЕЧЕНИЕ (проверь КАЖДЫЙ пункт):
${treatmentsList}

КРИТИЧЕСКИ ВАЖНО при анализе лечения:
1. Для химиотерапии — проверяй не только схему, но и ЧИСЛО КУРСОВ. Если число курсов не соответствует протоколу — это "сомнительно" или "не_рекомендовано". Укажи в comment рекомендуемое число курсов.
2. Для монотерапии — проверяй, не должна ли она применяться в КОМБИНАЦИИ с другими препаратами или с лучевой терапией. Если да — укажи это явно в comment.
3. Лучевая терапия часто применяется в комбинации с химиотерапией — проверяй наличие обоих компонентов.
4. Используй ТОЛЬКО актуальные ссылки: cr.minzdrav.gov.ru/preview-cr/... или rosoncoweb.ru/standarts/ или nccn.org/guidelines/

Для КАЖДОГО пункта лечения укажи:
- item: точное название
- status: "рекомендовано" / "сомнительно" / "не_рекомендовано"
- comment: подробный (2-3 предложения) комментарий с указанием что именно соответствует/не соответствует, какое число курсов рекомендовано, нужны ли комбинации
- source: "RUSSCO" или "Минздрав" или "NCCN"
- source_reference: актуальный URL только из cr.minzdrav.gov.ru/preview-cr/... или rosoncoweb.ru/standarts/ или nccn.org/guidelines/

В missing — виды лечения (включая лучевую терапию если нужна), которые должны были быть проведены но отсутствуют.
