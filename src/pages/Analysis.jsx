...
1. Определить нозологию по МКБ-коду: ${caseData.mkb_code || "не указан"}
2. Открыть актуальную клин. рекомендацию: ${guidelineUrl || "найти по МКБ на cr.minzdrav.gov.ru или rosoncoweb.ru"}
3. Найти раздел "2. Диагностика" в документе
4. Для каждого пункта из списка: найти соответствующий подпункт в разделе → сопоставить → вынести статус
5. Для missing: перечислить только те обследования, которые явно обозначены как обязательные (уровень доказательности I, II или A, B) в разделе диагностики, но отсутствуют

Для КАЖДОГО пункта диагностики:
- item: точное название из списка выше
- status: "рекомендовано" (есть в рекомендациях) / "сомнительно" (есть с оговорками) / "не_рекомендовано" (противоречит или не упомянуто)
- comment: FORMAT: "[Источник, раздел]: цитата из рекомендации. Данные пациента: значение. Вывод: совпадает/не совпадает."
- source: "RUSSCO" или "Минздрав"
- source_reference: точный URL (rosoncoweb.ru/standarts/ или cr.minzdrav.gov.ru/preview-cr/...)
...
ВХОДНЫЕ ДАННЫЕ ПАЦИЕНТА (только эти данные использовать для сверки):
${caseContext}
Стадия для поиска раздела лечения: ${caseData.tumor_stage || "не указана"} | TNM: T${caseData.t_stage || "?"}N${caseData.n_stage || "?"}M${caseData.m_stage || "?"}

ПРОВЕДЁННОЕ ЛЕЧЕНИЕ — проверить КАЖДЫЙ пункт, ни один не пропускать:
${treatmentsList}

АЛГОРИТМ (выполнять строго):
1. Открыть клин. рекомендацию: ${guidelineUrl || "найти по МКБ на cr.minzdrav.gov.ru или rosoncoweb.ru"}
2. Найти раздел "3. Лечение" → подраздел для данной стадии/линии терапии
3. Для каждого пункта лечения: сопоставить схему + число курсов + комбинации с конкретным подпунктом рекомендации
4. Число курсов проверять обязательно: если не совпадает с рекомендацией → "сомнительно" или "не_рекомендовано"
5. Монотерапия вместо обязательной комбинации → "не_рекомендовано"
6. Для missing: только явно рекомендованные компоненты для данной стадии/линии, которых нет в списке

Для КАЖДОГО пункта лечения:
- item: точное название из списка выше
- status: "рекомендовано" / "сомнительно" / "не_рекомендовано"
- comment: FORMAT: "[Источник, раздел X.X]: цитата из рекомендации (схема, число курсов). Данные пациента: [значение]. Вывод: совпадает/не совпадает по [конкретному параметру]."
- source: "RUSSCO" или "Минздрав" или "NCCN"
- source_reference: точный URL (cr.minzdrav.gov.ru/preview-cr/... или rosoncoweb.ru/standarts/ или nccn.org/guidelines/)
...
ЗАДАЧА: Нормативное заключение — только факты, только данные из документа и утверждённых источников.
- Запрещены предположения, прогнозы, альтернативы, интерпретации
- overall_compliance: "высокое" (≥80% соответствуют), "среднее" (50-79%), "низкое" (<50%)
- summary: 3-4 предложения строго по данным: что указано в документе, что соответствует рекомендациям, что не указано или требует дополнения
