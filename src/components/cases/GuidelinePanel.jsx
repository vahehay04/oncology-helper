...
export default function GuidelinePanel() {
  ...
  const handleAsk = async () => {
    ...
    const prompt = `Ты — медицинский справочный инструмент для врача-онколога. Ищи информацию СТРОГО в трёх источниках:
...
## ЗАПРОС ВРАЧА:
${query}${fileContext}
${uploadedFileUrl ? "\n[Приложен документ — учти его содержимое]" : ""}

## ЗАДАЧА:
1. Извлеки из запроса: диагноз, молекулярные маркеры, предшествующее лечение, текущую линию терапии.
2. Найди в указанных трёх источниках конкретные положения, касающиеся данного случая.
3. По каждому значимому аспекту укажи: соответствует / не соответствует / требует уточнения.
4. guideline_text — точная формулировка из источника.
5. comment — вывод о соответствии.
6. recommendation — конкретная рекомендация из источников.
7. sources — название документа, год, раздел.`;
