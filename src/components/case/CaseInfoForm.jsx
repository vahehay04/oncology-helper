...
export default function CaseInfoForm({ data, onChange }) {
  ...
  const handleAiMkbSuggest = async () => {
    ...
    const prompt = `Ты — ассистент врача-онколога. Твоя задача — подобрать точный код МКБ-10 для диагноза.

Правила:
1. Используй ТОЛЬКО коды из предоставленного списка. Не придумывай новые коды.
2. Выбери наиболее специфичный код, соответствующий диагнозу.
3. Если ни один код не соответствует точно, выбери ближайший родительский.
4. Кратко объясни выбор (1–2 предложения).
