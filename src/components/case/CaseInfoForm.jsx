...
export default function CaseInfoForm({ data, onChange }) {
  ...
  const handleAiMkbSuggest = async () => {
    ...
    const prompt = `Ты — система глубокого нормативного анализа для врача-онколога. Твоя задача — подобрать ТОЧНЫЙ код МКБ-10 для диагноза, опираясь на полную классификацию сайта https://rosoncoweb.ru.

## ОБЯЗАТЕЛЬНОЕ ПОВЕДЕНИЕ

Перед формированием ответа ты ОБЯЗАН:
1. Полностью изучить соответствующий раздел https://rosoncoweb.ru для данной нозологии.
2. Проверить наличие подклассификаций МКБ-10 (например: C15.0, C15.1, C15.2 и т.д.).
3. Изучить TNM-классификации, стадии, морфологические варианты, биомаркеры.
4. НЕ ограничиваться общим кодом — искать максимально точную подкатегорию.

## ПРАВИЛА ВЫБОРА КОДА

1. Используй ТОЛЬКО коды из предоставленного списка. Не придумывай новые коды.
2. Выбери НАИБОЛЕЕ СПЕЦИФИЧНЫЙ код (с наибольшим числом знаков после точки).
3. Если данных недостаточно для выбора подкатегории — выбери ближайший родительский код И обязательно укажи, какие параметры нужно уточнить.
4. НИКОГДА не делай вывод по неполным данным без указания недостающих параметров.
5. Учитывай особенности конкретной нозологии (не применяй универсальный шаблон).

## ЕСЛИ ДАННЫХ НЕДОСТАТОЧНО

Если для точного кода не хватает информации, в поле "missing_params" перечисли конкретные недостающие данные:
- Точная локализация опухоли
- Гистологический тип
- Степень дифференцировки (G1/G2/G3)
- Стадия TNM (T, N, M)
- Наличие метастазов
- Биомаркеры (если применимо для данной нозологии)
- Любые специфические параметры данного вида рака
...
Ответь строго в формате JSON:
{
  ...
  "reasoning": "ОБОСНОВАНИЕ ВЫБОРА со ссылкой на классификацию rosoncoweb.ru",
  "missing_params": ["параметр 1", "параметр 2"] или [] если данных достаточно,
