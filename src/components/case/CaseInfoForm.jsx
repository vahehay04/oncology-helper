...
Верни JSON. Если данных нет — "" или [].`;
    ...
    if (detectedType && detectedType.fields.length > 0) {
      ...
      const specificPrompt = `РЕЖИМ: СТРОГОЕ НОРМАТИВНОЕ ИЗВЛЕЧЕНИЕ СПЕЦИФИЧЕСКИХ ПАРАМЕТРОВ.

ПРАВИЛА (нарушение недопустимо):
- Извлекай ТОЛЬКО то, что ЯВНО написано в документе.
- Запрещено предполагать значение параметра если он не указан явно.
- Если параметр не найден в документе — верни пустую строку "".
- Сопоставляй найденное значение с предложенными вариантами, выбирай НАИБОЛЕЕ ТОЧНОЕ соответствие.
- Запрещено выбирать вариант "по умолчанию" если значение не найдено в документе.

НОЗОЛОГИЯ: ${detectedType.label}

ПАРАМЕТРЫ ДЛЯ ПОИСКА В ДОКУМЕНТЕ:
${fieldInstructions}

ИНСТРУКЦИЯ ПО ПОИСКУ: Ищи в документе следующие термины и их вариации:
- HER2: "HER2", "HER-2", "HER2/neu", "c-erbB2", упоминания "2+" "3+" "FISH" "ISH"
- MSI/MMR: "MSI", "MSS", "MMR", "dMMR", "pMMR", "MSI-H", "микросателлитная"
- PD-L1: "PD-L1", "CPS", "TPS", "22C3", "SP142", "SP263"
- FGFR2: "FGFR2", "FGFR", "амплификация"
- Тип по Lauren: "Lauren", "кишечный", "диффузный", "смешанный", "интестинальный"
- Гистология: "аденокарцинома", "плоскоклеточный", "перстневидноклеточный", "муцинозный", "степень дифференцировки", "G1" "G2" "G3"${docContext}

Верни JSON только с этими ключами. Пустая строка если параметр не найден.`;
