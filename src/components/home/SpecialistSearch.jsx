...
ШАГ 0 — ИЗВЛЕЧЕНИЕ ДАННЫХ ИЗ ДОКУМЕНТА (выполни ПЕРВЫМ)
Если выше есть раздел "СОДЕРЖИМОЕ ПРИЛОЖЕННОГО ДОКУМЕНТА", извлеки из него строго по следующим полям (если поле не найдено — оставь пустым, не придумывай):
- Диагноз (нозология, локализация, гистология)
- Стадия TNM (T, N, M, общая стадия)
- Молекулярные маркеры (HER2, PD-L1, MSI, EGFR, ALK, ROS1, BRAF, KRAS и т.д.)
- Линии лечения (все проведённые курсы, схемы, препараты, даты)
- Хирургические вмешательства
- Лучевая терапия
- Сопутствующие заболевания, ECOG
Только после этого используй извлечённые данные в последующих шагах. НЕ добавляй данные, которых нет в документе.
  ...
  "compliance_items": [
    {
      ...
      const res = await base44.integrations.Core.InvokeLLM({
        prompt: `ТЫ — справочная онкологическая система. Ты обладаешь знаниями клинических рекомендаций RUSSCO (rosoncoweb.ru), Минздрава РФ (cr.minздrav.gov.ru) и NCCN (nccn.org).
Отвечай строго на основе этих знаний. Работай детерминированно: одинаковый вопрос → всегда одинаковый ответ.

ВАЖНО: Если к вопросу приложен документ (раздел "СОДЕРЖИМОЕ ПРИЛОЖЕННОГО ДОКУМЕНТА"), сначала извлеки из него только те данные, которые явно присутствуют в тексте — дословно. Не интерпретируй и не дополняй. Затем используй их в ответе.
