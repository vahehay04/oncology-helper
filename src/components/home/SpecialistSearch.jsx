  ...
  const handleAsk = async () => {
    ...
    if (fullCase) {
      ...
      const res = await base44.integrations.Core.InvokeLLM({
        prompt: `Ты — строго нормативный клинический аналитик. Анализируешь клинический случай и сопоставляешь с официальными рекомендациями RUSSCO (rosoncoweb.ru) и Минздрава РФ (cr.minzdrav.gov.ru).
...
2. Для каждого клинически значимого аспекта найди в источниках (RUSSCO / Минздрав РФ) конкретный текст.
3. В поле "quote" приводи ДОСЛОВНУЮ ЦИТАТУ из рекомендации — именно те слова, которые написаны в документе. Не пересказывай, цитируй.
4. В поле "comment" — кратко объясни соответствует/не соответствует и почему.
5. В поле "source_name" — название документа (например: "Клин. рек. RUSSCO 2025, Рак пищевода").
6. В поле "source_url" — точный URL документа.
  ...
  "compliance_items": [
    {
      ...
      "quote": "дословная цитата из рекомендации",
      "comment": "клинический комментарий",
      "source_name": "название документа",
      "source_url": "https://..."
    }
  ]
}`,
      ...
      const res = await base44.integrations.Core.InvokeLLM({
        prompt: `Ты — нормативный медицинский справочник для врача-онколога. Отвечаешь ТОЛЬКО на основе официальных источников: RUSSCO (rosoncoweb.ru/standarts/), Минздрав РФ (cr.minzdrav.gov.ru), NCCN (nccn.org).

ВОПРОС ВРАЧА:
${query}${fileContext}

ПРАВИЛА:
- Если вопрос о схеме/протоколе (FLOT, XELOX, FOLFOX и т.д.) — дай состав с дозами и режимом введения дословно из рекомендации.
- Если вопрос о диагностике — перечисли обследования по рекомендациям с уровнями доказательности.
- Если вопрос о показаниях — процитируй точные критерии из документа.
- ОБЯЗАТЕЛЬНО: для каждого пункта ответа укажи дословную цитату из источника.
- Отвечай структурированно и конкретно. Краткий ответ с цитатами — лучше длинного текста.
  ...
  "answer_blocks": [
    {
      ...
      "quote": "дословная цитата из рекомендации (если есть)",
      "source_name": "название документа",
      "source_url": "https://..."
    }
  ]
