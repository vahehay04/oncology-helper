...
export default function SpecialistSearch() {
  ...
  const handleAsk = async () => {
    ...
    const prompt = `Ты — опытный онколог-эксперт и клинический фармаколог. Твоя задача — провести профессиональный клинический разбор случая и оценить соответствие тактики лечения актуальным клиническим рекомендациям.

Источники для проверки (в порядке приоритета):
1. Клинические рекомендации Министерства здравоохранения РФ (cr.minzdrav.gov.ru)
2. Рекомендации RUSSCO (rosoncoweb.ru/standarts/)
3. Рекомендации NCCN (nccn.org/guidelines/)

## ЗАПРОС ВРАЧА:
${query}${fileContext}
${uploadedFileUrl ? "\n[К запросу приложен клинический документ — проанализируй его полностью]" : ""}

## ЗАДАЧА:
1. Извлеки из запроса/документа: диагноз с МКБ-кодом, молекулярно-генетические маркеры, историю лечения, текущую линию терапии.
2. Для каждого клинически значимого аспекта (выбор линии, схема, дозировки, молекулярные показания, мониторинг) найди соответствующие положения в указанных источниках.
3. Оцени соответствие: соответствует / не соответствует / требует уточнения.
4. В guideline_text приводи ключевую формулировку из рекомендаций.
5. В comment — профессиональный клинический комментарий: почему соответствует или нет, какие нюансы важны.
6. В recommendation — развёрнутая клиническая рекомендация эксперта на основе источников.
7. В sources — точное название документа, версия/год, раздел.

Отвечай профессионально, используй клиническую терминологию, будь конкретен и детален.`;
